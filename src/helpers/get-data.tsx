...
export async function getDataSheet(personalData: IPersonalData) {
    if (!(personalData.googleEmail && personalData.googleTableId && personalData.googleKey)) {
    ...
    const categories = category_rows?.map(({ _sheet, _rowNumber, _rawData, ...fields }) => ({
        ...fields,
    }));

    const products = products_rows?.map(({ _sheet, _rowNumber, _rawData, ...fields }) => ({
        ...fields,
    }));

    const localization = localization_rows?.map(({ _sheet, _rowNumber, _rawData, ...fields }) => ({
        ...fields,
    }));
