...
export const useSearchable = (
    ...
    return useMemo(() => {
        ...
        let _data = data;
        if (category) {
            _data = _data.filter((item) => item.category_ids.includes(category));
        }
                ...
                if (filters[filtersName[i]].length > 0) {
                    _data = _data.filter((item) => {
                        ...
                        filters[filtersName[i]].forEach((elem) => {
                            if (item.params[filtersName[i]] && item.params[filtersName[i]].value.includes(elem)) {
                                equal = true;
                            }
                        });
        ...
        if (price?.length > 0) {
            _data = _data.filter((item) => item.price >= price[0] && item.price <= price[1]);
        }

        _data = _data.filter((item) => searchProps(item).some((sp) => regex.test(sp)));
